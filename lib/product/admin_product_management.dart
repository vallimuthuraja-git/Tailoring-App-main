...
class _AdminProductManagementState extends State<AdminProductManagement>
      ...
      providers: [
        ChangeNotifierProvider<ProductProvider>(
          create: (_) => ProductProvider(
              null), // Using basic demo data since BLoC is temporarily disabled
        ),
      ],
      child: Scaffold(
        appBar: AppBar(
    ...
    return Consumer<ProductProvider>(
      builder: (context, productProvider, child) {
        if (productProvider.isLoading) {
          return const Center(child: CircularProgressIndicator()); // TEMP
        }
        ...
        return Column(
          children: [
            ...
            Expanded(
              child: const Center(child: Text('Grid view TBD')), // TEMP
            ),
          ],
        );
      },
  ...
  Widget _buildAnalyticsTab() {
    // Temporarily using dummy data since analytics service is disabled
    final metrics = {
      'total_products': 2,
      'new_products_30_days': 1,
      'featured_products': 1,
      'average_rating': 4.65,
      'total_sold': 38,
    ...
    return ListView(
      ...
      children: [
        ...
        _buildAnalyticsCard('Average Rating',
            (metrics['average_rating'] as double).toStringAsFixed(1)),
        _buildAnalyticsCard('Total Sold', metrics['total_sold'].toString()),
        _buildAnalyticsCard('Total Revenue',
            '\$${(metrics['total_revenue'] as double).toStringAsFixed(2)}'),
      ],
    );
  }
