...
class ComprehensiveDemoDataService {
  ...
  static List<Customer> _generateCustomers() {
    ...
    final firstNames = ['Rajesh', 'Priya', 'Amit', 'Kavita', 'Vikram', 'Anjali', 'Rahul', 'Sneha', 'Arun', 'Meera', 'Suresh', 'Kiran', 'Deepak', 'Poonam', 'Rakesh', 'Sunita', 'Manoj', 'Rekha', 'Vinay', 'Geeta'];
    final lastNames = ['Kumar', 'Sharma', 'Patel', 'Singh', 'Gupta', 'Jain', 'Agarwal', 'Verma', 'Chauhan', 'Yadav', 'Shukla', 'Mishra', 'Tiwari', 'Chandra', 'Bansal', 'Kapoor', 'Mehta', 'Rao', 'Nair', 'Pillai'];
    ...
    for (int i = 0; i < _customerCount; i++) {
      ...
      final totalSpent = (_random.nextDouble() * 50000 + 5000) * spendingMultiplier;
      ...
      customers.add(Customer(
        ...
        email: email,
        phone: phone,
        photoUrl: 'https://via.placeholder.com/100x100?text=${firstName[0]}${lastName[0]}',
  ...
  static List<Product> _generateProducts() {
    ...
    for (int i = 0; i < _productCount; i++) {
      final template = productTemplates[_random.nextInt(productTemplates.length)];
      ...
      products.add(Product(
        ...
        category: category,
        basePrice: (template['basePrice'] as double) * (0.8 + _random.nextDouble() * 0.4), // ±20% variation
        imageUrls: [
  ...
  static List<String> _generateCustomerPreferences(bool isMale) {
    ...
    final colorPrefs = ['Dark Colors', 'Light Colors', 'Bright Colors', 'Pastel Colors', 'Neutral Colors'];
  ...
  static List<Map<String, dynamic>> _getProductTemplates() {
    return [
      {
        'name': 'Business Suit',
        'description': 'Professional business suit perfect for corporate environments',
        'category': ProductCategory.mensWear,
        'basePrice': 25000.0,
      },
      {
        'name': 'Wedding Lehenga',
        'description': 'Elegant wedding lehenga with intricate embroidery and traditional design',
        'category': ProductCategory.womensWear,
        'basePrice': 45000.0,
      },
      {
        'name': 'Cotton Shirt',
        'description': 'Comfortable cotton shirt ideal for casual and formal wear',
        'category': ProductCategory.mensWear,
        'basePrice': 3000.0,
      },
      {
        'name': 'Designer Dress',
        'description': 'Stylish designer dress for special occasions and parties',
        'category': ProductCategory.womensWear,
        'basePrice': 15000.0,
      },
      {
      ...
      },
      {
        ...
        'description': 'Elegant traditional sherwani for weddings and festivals',
  ...
  static Map<String, dynamic> _generateProductSpecifications(ProductCategory category) {
   ...
   static List<Employee> _generateEmployees() {
     ...
     for (int i = 0; i < _employeeCount - 1; i++) {
       ...
       final lastName = lastNames[_random.nextInt(lastNames.length)];
       final name = '$firstName $lastName';
       final email = '${firstName.toLowerCase()}.${lastName.toLowerCase()}@tailoring.com';
       ...
       final availability = EmployeeAvailability.values[i % EmployeeAvailability.values.length];
       ...
       final performanceBonusRate = baseRate * 0.1 * (_random.nextDouble() + 0.5);
       ...
       employees.add(Employee(
         ...
         canWorkRemotely: availability == EmployeeAvailability.remote || _random.nextDouble() > 0.7,
         ...
         totalEarnings: (totalOrdersCompleted * baseRate * 8) + (_random.nextDouble() * 50000),
         ...
         isActive: _random.nextDouble() > 0.1, // 90% active
         joinedDate: DateTime.now().subtract(Duration(days: _random.nextInt(730))), // Up to 2 years ago
   ...
   static List<String> _generateSpecializations(List<EmployeeSkill> skills) {
     ...
     final specializationOptions = {
       EmployeeSkill.cutting: ['Precision Cutting', 'Fabric Optimization', 'Pattern Cutting'],
       EmployeeSkill.stitching: ['Hand Stitching', 'Machine Sewing', 'Complex Stitches'],
       EmployeeSkill.finishing: ['Button Attachment', 'Hemming', 'Final Touches'],
       EmployeeSkill.alterations: ['Size Modifications', 'Style Changes', 'Fit Adjustments'],
       EmployeeSkill.embroidery: ['Machine Embroidery', 'Hand Embroidery', 'Custom Designs'],
       EmployeeSkill.qualityCheck: ['Quality Assurance', 'Final Inspection', 'Standards Compliance'],
       EmployeeSkill.patternMaking: ['Custom Patterns', 'Size Grading', 'Technical Drawing'],
     };

     for (final skill in skills) {
       final skillSpecializations = specializationOptions[skill]!;
       final specializationCount = _random.nextInt(2) + 1; // 1-2 specializations per skill

       for (int i = 0; i < specializationCount; i++) {
         specializations.add(skillSpecializations[_random.nextInt(skillSpecializations.length)]);
       }
     }
   ...
   static List<String> _generateCertifications(List<EmployeeSkill> skills, int experienceYears) {
     final certifications = <String>[];
     final possibleCertifications = [
       'Certified Tailor', 'Fashion Design Diploma', 'Sewing Machine Specialist',
       'Quality Assurance Certification', 'Embroidery Professional', 'Pattern Making Expert',
       'Alteration Specialist', 'Textile Technology Certification', 'Fashion Merchandising'
     ];

     final certificationCount = experienceYears > 5 ? _random.nextInt(3) + 1 : _random.nextInt(2); // 0-3 certifications

     for (int i = 0; i < certificationCount; i++) {
       certifications.add(possibleCertifications[_random.nextInt(possibleCertifications.length)]);
     }
   ...
   static List<String> _generateWorkDays(EmployeeAvailability availability) {
     final allDays = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
   ...
   static Map<String, TimeOfDay?> _generateWorkHours(EmployeeAvailability availability) {
   ...
   static String? _generateWorkLocation(EmployeeAvailability availability) {
     final locations = ['Main Shop', 'Branch Office', 'Home Office', 'Client Location'];
     ...
     if (availability == EmployeeAvailability.remote) {
       ...
       return _random.nextBool() ? locations[0] : locations[1]; // Main shop or branch
     }
   }

   static double _calculateBaseRate(List<EmployeeSkill> skills, int experienceYears) {
     ...
     return 100.0 + skillBonus + experienceBonusAmount + (_random.nextDouble() * 50); // Base 100-150 + bonuses
   }

   static List<String> _generateStrengths(List<EmployeeSkill> skills) {
     final possibleStrengths = [
       'Attention to Detail', 'Speed and Efficiency', 'Creative Problem Solving',
       'Customer Service', 'Quality Focus', 'Technical Expertise', 'Adaptability',
       'Team Collaboration', 'Time Management', 'Precision Work'
     ];
     ...
     for (int i = 0; i < strengthCount; i++) {
       strengths.add(possibleStrengths[_random.nextInt(possibleStrengths.length)]);
     }
   ...
   static List<String> _generateAreasForImprovement() {
     final possibleAreas = [
       'Technology Adoption', 'Advanced Techniques', 'Speed Optimization',
       'Client Communication', 'Documentation', 'Process Standardization'
     ];
   ...
   static List<Service> _generateServices() {
     ...
     for (int i = services.length; i < services.length + additionalServiceCount; i++) {
       final category = ServiceCategory.values[i % ServiceCategory.values.length];
       ...
       services.add(Service(
         ...
         tierPricing: serviceTemplate['tierPricing'] as Map<String, double>? ?? {},
         ...
         preparationTips: serviceTemplate['preparationTips'] as List<String>? ?? [],
         specifications: serviceTemplate['specifications'] as Map<String, dynamic>? ?? {},
         customizations: serviceTemplate['customizations'] as List<ServiceCustomization>? ?? [],
         addOns: serviceTemplate['addOns'] as List<String>? ?? [],
         addOnPricing: serviceTemplate['addOnPricing'] as Map<String, double>? ?? {},
         isActive: _random.nextDouble() > 0.1, // 90% active
         requiresMeasurement: serviceTemplate['requiresMeasurement'] as bool? ?? false,
         requiresFitting: serviceTemplate['requiresFitting'] as bool? ?? false,
         estimatedHours: serviceTemplate['estimatedHours'] as int? ?? 1,
         requiredSkills: serviceTemplate['requiredSkills'] as List<String>? ?? [],
         recommendedFabrics: serviceTemplate['recommendedFabrics'] as List<String>? ?? [],
   ...
   static Map<String, dynamic> _getServiceTemplateForCategory(ServiceCategory category, int index) {
     final templates = _getServiceTemplatesByCategory()[category]!;
     return templates[index % templates.length];
   }

   static Map<ServiceCategory, List<Map<String, dynamic>>> _getServiceTemplatesByCategory() {
     return {
       ServiceCategory.sareeServices: [
         {
           'id': 'saree_falls_pinning_${_random.nextInt(1000)}',
           'name': 'Saree Falls Pinning',
           'description': 'Professional saree falls pinning service to secure saree pleats perfectly. Includes multiple pinning techniques and safety measures.',
           ...
           'basePrice': 15.0,
           'features': ['Secure pinning', 'Multiple techniques', 'Safety pins included', 'Quick service'],
         ...
         },
         {
           ...
           'description': 'Complete saree alteration service including length adjustment, width modification, and style changes.',
           ...
           'minPrice': 30.0,
           'maxPrice': 120.0,
           'features': ['Length adjustment', 'Width modification', 'Hem stitching', 'Style alterations'],
       ...
       ServiceCategory.garmentServices: [
         {
           ...
           'description': 'Bespoke dress creation service with custom design, fabric selection, and multiple fittings.',
           ...
           'minPrice': 200.0,
           'maxPrice': 800.0,
           'tierPricing': {'Basic': 200.0, 'Standard': 300.0, 'Premium': 500.0, 'Luxury': 800.0},
           'features': ['Custom design', 'Fabric selection', 'Multiple fittings', 'Alterations included'],
           'requiresMeasurement': true,
         ...
         },
         {
           ...
           'description': 'Professional suit tailoring service with custom measurements, fabric selection, and expert craftsmanship.',
           ...
           'minPrice': 350.0,
           'maxPrice': 1500.0,
           'features': ['Custom measurements', 'Fabric consultation', 'Expert tailoring', 'Multiple fittings'],
           'requiresMeasurement': true,
           'requiresFitting': true,
           'estimatedHours': 60,
         ...
         },
         {
           ...
           'description': 'Custom shirt creation with personalized fit, fabric choice, and style customization.',
           ...
           'minPrice': 80.0,
           'maxPrice': 300.0,
           'features': ['Custom fit', 'Fabric selection', 'Collar options', 'Cuff styles'],
           'requiresMeasurement': true,
           'requiresFitting': true,
           'estimatedHours': 16,
       ...
       ServiceCategory.alterationServices: [
         {
           ...
           'description': 'Expert hemming service for pants, skirts, dresses, and curtains. Includes different hemming techniques and finishes.',
           ...
           'minPrice': 15.0,
           'maxPrice': 75.0,
           'features': ['Professional hemming', 'Multiple techniques', 'Invisible stitches', 'Quick turnaround'],
           'requiresMeasurement': false,
           'estimatedHours': 2,
         ...
         },
         {
           ...
           'description': 'Professional taking in and letting out service for perfect fit adjustments.',
           ...
           'minPrice': 25.0,
           'maxPrice': 100.0,
           'features': ['Size adjustment', 'Seam ripping', 'Re-stitching', 'Fit guarantee'],
           'requiresMeasurement': true,
           'estimatedHours': 4,
       ...
       ServiceCategory.customDesign: [
         {
           ...
           'description': 'Complete bespoke design service including concept development, fabric selection, and custom creation.',
           ...
           'minPrice': 500.0,
           'maxPrice': 2500.0,
           'features': ['Concept development', 'Design sketches', 'Fabric selection', 'Custom creation', 'Multiple fittings'],
           'requiresMeasurement': true,
           'requiresFitting': true,
           'estimatedHours': 80,
       ...
       ServiceCategory.consultation: [
         {
           ...
           'description': 'Professional style consultation to help you choose the perfect outfit for any occasion.',
           'shortDescription': 'Professional style consultation',
           'type': ServiceType.styleConsultation,
           'duration': ServiceDuration.quick,
           'complexity': ServiceComplexity.simple,
           'basePrice': 75.0,
           'features': ['Personal style assessment', 'Color analysis', 'Body type analysis', 'Occasion planning', 'Shopping recommendations'],
           'requiresMeasurement': false,
           'estimatedHours': 2,
       ...
       ServiceCategory.specialOccasion: [
         {
           ...
           'description': 'Complete wedding dress creation service with custom design, premium fabrics, and expert craftsmanship.',
           ...
           'minPrice': 800.0,
           'maxPrice': 3500.0,
           'features': ['Custom design', 'Premium fabrics', 'Multiple fittings', 'Alterations included', 'Rush service available'],
           'requiresMeasurement': true,
           'requiresFitting': true,
           'estimatedHours': 120,
           'requiredSkills': ['wedding_dress_design', 'bridal_tailoring', 'premium_finishing'],
       ...
       ServiceCategory.corporateWear: [
         {
           ...
           'description': 'Professional corporate suit creation with business attire standards and quality materials.',
           ...
           'minPrice': 300.0,
           'maxPrice': 1200.0,
           'features': ['Business standards', 'Quality materials', 'Professional fit', 'Multiple fittings'],
           'requiresMeasurement': true,
           'requiresFitting': true,
           'estimatedHours': 50,
       ...
       ServiceCategory.uniformServices: [
         {
           ...
           'description': 'Complete school uniform service including custom sizing, durable materials, and school compliance.',
           ...
           'minPrice': 120.0,
           'maxPrice': 400.0,
           'features': ['Custom sizing', 'Durable materials', 'School compliance', 'Bulk orders'],
           'requiresMeasurement': true,
           'requiresFitting': true,
           'estimatedHours': 24,
       ...
       ServiceCategory.bridalServices: [
         {
           ...
           'description': 'Exquisite bridal gown creation with premium materials, intricate detailing, and perfect fit.',
           ...
           'minPrice': 1000.0,
           'maxPrice': 4000.0,
           'features': ['Premium materials', 'Intricate detailing', 'Perfect fit', 'Alterations included'],
           'requiresMeasurement': true,
           'requiresFitting': true,
           'estimatedHours': 140,
           'requiredSkills': ['bridal_design', 'premium_tailoring', 'intricate_work'],
       ...
       ServiceCategory.measurements: [
         {
           ...
           'description': 'Comprehensive body measurement service for accurate tailoring and perfect fit.',
           'shortDescription': 'Professional body measurement service',
           'type': ServiceType.bodyMeasurement,
           'duration': ServiceDuration.quick,
           'complexity': ServiceComplexity.simple,
           'basePrice': 35.0,
           'features': ['Comprehensive measurements', 'Digital recording', 'Measurement chart', 'Fit recommendations'],
           'requiresMeasurement': false,
           'estimatedHours': 1,
   ...
   static List<String> _generateServiceImageUrls(String serviceName) {
     final baseName = serviceName.toLowerCase().replaceAll(' ', '_').replaceAll(RegExp(r'[^\w_]'), '');
     return [
  ...
  static Map<String, dynamic> _generateAdditionalEmployeeInfo() {
    ...
    if (_random.nextBool()) {
      info['preferredPaymentMethod'] = _random.nextBool() ? 'Bank Transfer' : 'Cash';
    }

    if (_random.nextBool()) {
      info['languages'] = ['English', 'Hindi', 'Marathi', 'Gujarati', 'Punjabi'].take(_random.nextInt(3) + 1).toList();
    }
   ...
   static Future<void> _populateWorkAssignments() async {
     ...
     for (final assignment in workAssignments) {
       final docRef = _firestore.collection('work_assignments').doc(assignment.id);
     ...
     for (int i = 0; i < _workAssignmentCount && i < orders.length; i++) {
       ...
       if (assignedEmployee != null) {
         ...
         final assignment = WorkAssignment(
           ...
           assignedAt: DateTime.now().subtract(Duration(days: _random.nextInt(30))),
           ...
           deadline: DateTime.now().add(Duration(days: _random.nextInt(21) + 7)), // 7-28 days
   ...
   static Employee? _findSuitableEmployeeForOrder(List<Employee> employees, Order order) {
     ...
     final suitableEmployees = employees.where((employee) {
       ...
       return order.items.any((item) =>
           _doesEmployeeHaveRequiredSkills(employee, item));
     }).toList();

     if (suitableEmployees.isEmpty) {
       // If no perfect match, pick a random employee
       return employees.isNotEmpty ? employees[_random.nextInt(employees.length)] : null;
     }

     return suitableEmployees[_random.nextInt(suitableEmployees.length)];
   }

   static bool _doesEmployeeHaveRequiredSkills(Employee employee, OrderItem item) {
   ...
   static Employee _createShopOwnerEmployee() {
     return Employee(
       ...
       averageRating: 4.8,
       completionRate: 0.95,
       strengths: ['Leadership', 'Customer Relations', 'Quality Control', 'Business Management'],
       areasForImprovement: [],
       ...
       isActive: true,
       joinedDate: DateTime.now().subtract(const Duration(days: 1825)), // 5 years ago
       additionalInfo: {
         'position': 'Shop Owner',
         'responsibilities': ['Shop Management', 'Customer Relations', 'Employee Supervision', 'Quality Assurance'],
       },
   ...
   static EmployeeSkill _determineRequiredSkillForOrder(Order order) {
     ...
     for (final item in order.items) {
       ...
       if (itemName.contains('suit') || itemCategory.contains('suit')) {
         ...
         return EmployeeSkill.stitching;
       } else if (itemName.contains('alteration') || itemCategory.contains('alteration')) {
         return EmployeeSkill.alterations;
       } else if (itemName.contains('lehenga') || itemCategory.contains('lehenga')) {
   ...
   static Future<void> _populateChatConversations() async {
     ...
     for (final conversation in conversations) {
       final docRef = _firestore.collection('chat_conversations').doc(conversation.id);
   ...
   static List<ChatConversation> _generateChatConversations() {
     ...
     final customers = _generateCustomers().take(_chatConversationCount).toList();
     ...
     for (int i = 0; i < _chatConversationCount && i < customers.length; i++) {
       ...
       final conversation = ChatConversation(
         ...
         lastMessageTime: DateTime.now().subtract(Duration(minutes: _random.nextInt(1440))), // Last 24 hours
         ...
         updatedAt: DateTime.now().subtract(Duration(minutes: _random.nextInt(1440))),
       );
   ...
   static Future<void> _populateChatMessages(List<ChatConversation> conversations) async {
     final batch = _firestore.batch();

     for (final conversation in conversations) {
       final messageCount = _random.nextInt(10) + 5; // 5-15 messages per conversation
       ...
       for (int i = 0; i < messageCount; i++) {
         final message = ChatMessage(
           ...
           timestamp: conversation.createdAt.add(Duration(minutes: i * 15 + _random.nextInt(30))),
           isRead: i < messageCount - _random.nextInt(3), // Last few messages might be unread
         );
   ...
   static MessageType _getRandomMessageType() {
     const types = [MessageType.text, MessageType.text, MessageType.text, MessageType.orderStatus];
     return types[_random.nextInt(types.length)];
   }

   static Map<String, dynamic>? _generateMessageMetadata() {
     if (_random.nextDouble() > 0.7) { // 30% chance of having metadata
       return {
         'intent': tailoringChatbotIntents[_random.nextInt(tailoringChatbotIntents.length)].intent,
  ...
  static List<String> _generateCustomizationOptions(ProductCategory category) {
    switch (category) {
      ...
      case ProductCategory.womensWear:
        return ['Length Adjustment', 'Sleeve Style', 'Neck Design', 'Color Choice'];
      case ProductCategory.customDesign:
        return ['Design Consultation', 'Fabric Selection', 'Style Customization', 'Accessories'];
      default:
