...
class WorkAssignmentService {
  ...
  Future<List<WorkAssignmentRecommendation>> getAssignmentRecommendations({
    ...
    try {
      ...
      for (final employee in employees) {
        ...
        if (suitability.suitabilityScore > 0.3) {
          // Only include reasonably suitable employees
  ...
  Future<WorkAssignmentRecommendation> _calculateSuitabilityScore({
    ...
    final availabilityScore =
        await _calculateAvailabilityScore(employee, deadline, estimatedHours);
    ...
    final performanceScore =
        _calculatePerformanceScore(employee, requiredSkills);
    ...
    final adjustedHours =
        _estimateWorkHours(employee, estimatedHours, requiredSkills);
  ...
  Future<double> _calculateAvailabilityScore(
    ...
    final maxWorkload =
        employee.availability == emp.EmployeeAvailability.fullTime ? 3 : 2;

    // Calculate workload factor
    final workloadFactor = currentWorkload >= maxWorkload
        ? 0.0
        : currentWorkload >= maxWorkload - 1
            ? 0.5
            : 1.0;
    ...
    final consecutiveDaysFactor = employee.consecutiveDaysWorked > 5
        ? 0.7
        : employee.consecutiveDaysWorked > 3
            ? 0.9
            : 1.0;

    return (workloadFactor * 0.5) +
        (timeFactor * 0.3) +
        (consecutiveDaysFactor * 0.2);
  }
  ...
  double _calculatePerformanceScore(
    ...
    final completionAdjustment =
        (employee.completionRate - 0.8) * 0.5; // Target 80% completion rate
  ...
  double _calculateExperienceScore(
    ...
    return (experienceScore + certificationBonus + specializationBonus)
        .clamp(0.0, 1.0);
  }

  double _calculatePreferenceScore(
      emp.Employee employee, Map<String, dynamic> orderDetails) {
    double score = 0.0;
    int factors = 0;
    ...
    if (orderType != null) {
      if (employee.specializations.any(
          (spec) => spec.toLowerCase().contains(orderType.toLowerCase()))) {
        score += 1.0;
        factors++;
      }
    }
  ...
  double _estimateWorkHours(
    ...
    final similarAssignments = employee.recentAssignments.where((assignment) {
      final assignmentSkill =
          assignment.requiredSkill.toString().split('.').last;
      return requiredSkills.any((skill) =>
          skill.toLowerCase().contains(assignmentSkill.toLowerCase()));
    }).toList();
    ...
    final avgEfficiency = similarAssignments
            .map((a) =>
                a.estimatedHours > 0 ? a.actualHours / a.estimatedHours : 1.0)
            .reduce((a, b) => a + b) /
        similarAssignments.length;
  ...
  Future<bool> autoAssignWork({
    ...
    try {
      ...
      final employeeRef =
          _firestore.collection('employees').doc(bestMatch.employeeId);
  ...
  Future<Map<String, dynamic>> getAssignmentAnalytics() async {
    try {
      ...
      final completedAssignments = assignments
          ...
          .toList();
      final autoAssigned = assignments
          .where((a) => a.assignedBy.toLowerCase().contains('auto'))
          .toList();
  ...
  Map<String, double> _calculateSkillEfficiency(
    ...
    for (final assignment in assignments) {
      ...
      skillStats[assignment.requiredSkill]!['totalHours'] +=
    ...
    return skillStats.map((skill, stats) {
      ...
      final avgHours = stats['completed'] > 0
          ? stats['totalHours'] / stats['completed']
          : 0.0;
  ...
  double _calculateAverageCompletionTime(
    ...
    final totalHours =
        completedAssignments.map((a) => a.actualHours).reduce((a, b) => a + b);

    return totalHours / completedAssignments.length;
  }

  List<String> _generateAssignmentOptimization(
      List<emp.WorkAssignment> assignments) {
    ...
    if (overdueAssignments > assignments.length * 0.1) {
      optimizations.add(
          'High overdue rate detected - consider adjusting time estimates');
    }
    ...
    if (skillCounts.isNotEmpty) {
      ...
      if (maxSkillCount > minSkillCount * 2) {
        optimizations
            .add('Skill utilization imbalance - consider cross-training');
      }
    }
