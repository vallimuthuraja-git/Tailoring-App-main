...
class EmployeeAnalyticsService {
  ...
  Future<Map<String, dynamic>> getEmployeeAnalytics(String employeeId) async {
    try {
      ...
      final employee =
          emp.Employee.fromJson({...employeeDoc.data()!, 'id': employeeDoc.id});
      ...
      final assignments = assignmentsQuery.docs
          .map((doc) =>
              emp.WorkAssignment.fromJson({...doc.data(), 'id': doc.id}))
          .toList();

      // Calculate analytics
      final completedAssignments = assignments
          .where((a) => a.status == emp.WorkStatus.completed)
          .toList();
      final overdueAssignments = assignments.where((a) => a.isOverdue).toList();
      final onTimeAssignments = assignments
          .where((a) => a.isOnTime && a.status == emp.WorkStatus.completed)
          .toList();
      ...
      return {
        ...
        'pendingAssignments': assignments
            .where((a) =>
                a.status == emp.WorkStatus.notStarted ||
                a.status == emp.WorkStatus.inProgress)
            .length,
        'overdueAssignments': overdueAssignments.length,
        'onTimeCompletionRate': assignments.isNotEmpty
            ? (onTimeAssignments.length / assignments.length) * 100
            : 0,
        'averageCompletionTime':
            _calculateAverageCompletionTime(completedAssignments),
  ...
  Future<Map<String, dynamic>> getTeamAnalytics() async {
    try {
      ...
      final assignmentsQuery =
          await _firestore.collection('work_assignments').get();
      ...
      final totalCompletedOrders =
          employees.fold(0, (total, e) => total + e.totalOrdersCompleted);
      final totalEarnings =
          employees.fold(0.0, (total, e) => total + e.totalEarnings);
      ...
      final workloadDistribution =
          _calculateWorkloadDistribution(employees, allAssignments);
      ...
      return {
        ...
        'utilizationRate':
            _calculateTeamUtilizationRate(employees, allAssignments),
  ...
  Future<Map<String, dynamic>> getWorkEfficiencyAnalytics() async {
    try {
      ...
      final averageCompletionTime =
          _calculateAverageCompletionTime(completedAssignments);
      final onTimeCompletionRate =
          completedAssignments.where((a) => a.isOnTime).length /
      ...
      final optimizationSuggestions =
          _generateOptimizationSuggestions(assignments, completedAssignments);
      ...
      return {
        ...
        'efficiencyScore':
            _calculateOverallEfficiency(assignments, completedAssignments),
      };
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> _getMonthlyPerformanceData(
    ...
    for (int i = 11; i >= 0; i--) {
      ...
      final completedCount = monthAssignments
          .where((a) => a.status == emp.WorkStatus.completed)
          .length;
      final totalEarnings =
          monthAssignments.fold(0.0, (total, a) => total + a.totalEarnings);
      ...
      monthlyData.add({
        ...
        'utilizationRate': monthAssignments.isNotEmpty
            ? (completedCount / monthAssignments.length) * 100
            : 0,
      });
    }
  ...
  Map<String, double> _calculateSkillUtilization(
      List<emp.WorkAssignment> assignments) {
    final skillUsage = <emp.EmployeeSkill, int>{};
    final skillEarnings = <emp.EmployeeSkill, double>{};

    for (final assignment in assignments) {
      skillUsage[assignment.requiredSkill] =
          (skillUsage[assignment.requiredSkill] ?? 0) + 1;
      skillEarnings[assignment.requiredSkill] =
          (skillEarnings[assignment.requiredSkill] ?? 0.0) +
              assignment.totalEarnings;
    }
  ...
  Map<String, dynamic> _calculateTimeAnalytics(
      List<emp.WorkAssignment> assignments) {
    final completedAssignments =
        assignments.where((a) => a.status == emp.WorkStatus.completed).toList();

    if (completedAssignments.isEmpty) {
      return {
        'averageHoursPerTask': 0.0,
    ...
    final totalHours =
        completedAssignments.fold(0.0, (total, a) => total + a.actualHours);
    final totalEstimatedHours =
        completedAssignments.fold(0.0, (total, a) => total + a.estimatedHours);
    final averageHoursPerTask = totalHours / completedAssignments.length;
    final efficiencyRate = totalEstimatedHours > 0
        ? (totalHours / totalEstimatedHours) * 100
        : 0.0;
  ...
  Map<String, dynamic> _calculateQualityAnalytics(
      List<emp.WorkAssignment> completedAssignments) {
    if (completedAssignments.isEmpty) {
      return {
        'averageQualityRating': 0.0,
        'qualityConsistency': 0.0,
        'topRatedSkills': []
      };
    }

    final ratedAssignments =
        completedAssignments.where((a) => a.qualityRating != null).toList();
    ...
    final variance =
        ratings.map((r) => (r - mean) * (r - mean)).reduce((a, b) => a + b) /
            ratings.length;
    final consistency = variance > 0
        ? 100 - (variance * 25)
        : 100; // Higher consistency = lower variance
  ...
  double _calculateEfficiencyScore(
      emp.Employee employee, List<emp.WorkAssignment> assignments) {
    ...
    final onTimeRate = completedAssignments.where((a) => a.isOnTime).length /
        completedAssignments.length;
    final qualityScore = employee.averageRating / 5.0; // Normalize to 0-1
    final utilizationRate =
        employee.ordersInProgress > 0 ? 1.0 : 0.5; // Bonus for active work
  ...
  double _calculateWorkloadBalance(
      emp.Employee employee, List<emp.WorkAssignment> assignments) {
  ...
  Map<String, dynamic> _calculateDepartmentBreakdown(
  ...
  Map<String, dynamic> _calculateWorkloadDistribution(
      List<emp.Employee> employees, List<emp.WorkAssignment> assignments) {
    final workloadData = <String, Map<String, dynamic>>{};

    for (final employee in employees) {
      final employeeAssignments =
          assignments.where((a) => a.employeeId == employee.id).toList();
      final completedCount = employeeAssignments
          .where((a) => a.status == emp.WorkStatus.completed)
          .length;
      ...
      workloadData[employee.displayName] = {
        ...
        'pendingAssignments': employeeAssignments
            .where((a) => a.status != emp.WorkStatus.completed)
            .length,
        'currentWorkload': employee.ordersInProgress,
        'completionRate': employeeAssignments.isNotEmpty
            ? (completedCount / employeeAssignments.length) * 100
            : 0.0,
      };
    }
  ...
  List<Map<String, dynamic>> _calculatePerformanceRankings(
      List<emp.Employee> employees) {
    return employees
        .map((employee) => {
              'id': employee.id,
              ...
              'efficiencyScore': _calculateEfficiencyScore(
                  employee, []), // Would need assignments
        ...
        .toList()
      ..sort((a, b) => (b['efficiencyScore'] as double)
          .compareTo(a['efficiencyScore'] as double));
  }
  ...
  double _calculateTeamUtilizationRate(
      List<emp.Employee> employees, List<emp.WorkAssignment> assignments) {
    if (employees.isEmpty) return 0.0;

    final activeEmployees = employees.where((e) => e.isActive).length;
    final employeesWithWork =
        assignments.map((a) => a.employeeId).toSet().length;

    return employeesWithWork / activeEmployees;
  }

  double _calculateCostEfficiency(
      List<emp.Employee> employees, List<emp.WorkAssignment> assignments) {
    if (assignments.isEmpty) return 0.0;

    final totalCost = assignments.fold(
        0.0, (total, a) => total + (a.hourlyRate * a.actualHours));
  ...
  double _calculateOvertimeRate(List<emp.WorkAssignment> assignments) {
    final overtimeAssignments =
        assignments.where((a) => a.actualHours > a.estimatedHours * 1.2).length;
    return assignments.isNotEmpty
        ? (overtimeAssignments / assignments.length) * 100
        : 0.0;
  }

  double _calculateReworkRate(List<emp.WorkAssignment> assignments) {
    final reworkAssignments =
        assignments.where((a) => a.status == emp.WorkStatus.rejected).length;
    return assignments.isNotEmpty
        ? (reworkAssignments / assignments.length) * 100
        : 0.0;
  }
  ...
  Future<List<Map<String, dynamic>>> _identifyBottlenecks(
    ...
    for (final assignment
        in assignments.where((a) => a.status == emp.WorkStatus.completed)) {
    ...
    final sortedSkills = skillDelays.entries
        ...
        .toList()
      ..sort((a, b) =>
          (b['averageDelay'] as double).compareTo(a['averageDelay'] as double));

    return sortedSkills.take(3).toList();
  }

  List<String> _generateOptimizationSuggestions(
      List<emp.WorkAssignment> allAssignments,
      List<emp.WorkAssignment> completedAssignments) {
    ...
    for (final assignment in allAssignments) {
      skillCounts[assignment.requiredSkill] =
          (skillCounts[assignment.requiredSkill] ?? 0) + 1;
    }
    ...
    final averageRating = completedAssignments
            ...
            .map((a) => a.qualityRating!)
            .reduce((a, b) => a + b) /
        completedAssignments.where((a) => a.qualityRating != null).length;

    if (averageRating < 3.5) {
      suggestions.add(
          'Quality ratings could be improved - consider additional training or quality checks');
    }

    return suggestions;
  }

  double _calculateOverallEfficiency(List<emp.WorkAssignment> allAssignments,
      List<emp.WorkAssignment> completedAssignments) {
    if (completedAssignments.isEmpty) return 0.0;
    ...
    return (completionRate * 0.4) +
        (onTimeRate * 0.4) +
        (_calculateTeamUtilizationRate([], allAssignments) * 0.2);
  }
}
